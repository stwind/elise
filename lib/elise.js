...
p.showStart = function (schema) {
  ...
  async.map(schema1, function (img, done) {
    fImage.create(img.url, img.props, function (image) {
      done(null, image);
    });
  }, function (err, images) {
    _.each(images, function (image) {
      var tween = createjs.Tween.get(image.container);

      _.each(image.frame.steps, function (step) {
        elise._tweenStep(tween, step)
      });
...
p._tweenStep = function (tween, step) {
  ...
  switch (step) {
    ...
    default:
      tween[step.f].apply(tween, step.p);
  }
};
