...
p.showStart = function (schema) {
  ...
  var schema1 = _.map(schema, function (props, url) {
    return {url: url, frame: props.frame, content: props.content};
  });

  async.map(schema1, function (img, done) {
    var image = fImage.create(img.url, img.frame);

    image.bitmap.image.onload = function () {
      done(null, {target: image, content: img.content});
    };
  }, function (err, bitmaps) {
    _.each(bitmaps, function (bitmap) {
      var tween = createjs.Tween.get(bitmap.target);

      _.each(bitmap.content, function (params, func) {
...
p.tweenStep = function (tween, func, params) {
